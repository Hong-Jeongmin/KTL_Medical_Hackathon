...
class Resnet(nn.Module):
    def __init__(self, ver=50):
        ...
        self.num_feature = architecture.fc.in_features
        ...
        #binary classifier
        self.g = nn.Sequential(nn.Linear(self.num_feature, 1), nn.Sigmoid())
...
class VGG(nn.Module):
    def __init__(self,  ver=19):
        ...
        for name, module in architecture.named_children():
            if name=='avgpool':
                module = nn.AdaptiveAvgPool2d(output_size=(1,1))
            if not name == 'classifier':
                self.f.append(module)

        #encoder
        self.f = nn.Sequential(*self.f)
        #binary classifier
        self.g = nn.Sequential(nn.Linear(512, 1), nn.Sigmoid())
...
class Densenet(nn.Module):
    def __init__(self, ver):
        ...
        self.num_feature = architecture.classifier.in_features
